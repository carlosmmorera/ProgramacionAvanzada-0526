...
class Order(BaseModel):
    user_id: int = Field(..., description="ID del usuario que realiza el pedido")
    product: str = Field(..., min_length=1, description="Nombre del producto")
    quantity: int = Field(1, ge=1, description="Cantidad (mínimo 1)")
    price: float = Field(..., gt=0, description="Precio unitario (mayor que 0)")
...
def create_order(order: Order):
    logger.info(f"📦 Pedido recibido: {order}")
    db = SessionLocal()
    ...
    if existing:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"El usuario ya tiene un pedido del producto '{order.product}'"
        )
    ...
    db.refresh(order_db)
    db.close()
    logger.info(f"✅ Pedido guardado: {order_db.product} para usuario {order_db.user_id}")
    return {
